...
@Component
public class DownloadMCPStableManifestTasklet implements Tasklet, InitializingBean {
    ...
    public RepeatStatus execute(final StepContribution contribution, final ChunkContext chunkContext) throws Exception {
        ...
        File manifestFile = new File(workingDir, Constants.MCP_STABLE_MAVEN_METADATA_FILE);
        if (manifestFile.exists())
            Assert.state(manifestFile.delete(), "Failed to delete the MCPConfig maven metadata file. It might be in use.");

        Utils.downloadFile(manifestFile.toPath(), Constants.MCP_STABLE_MAVEN_METADATA_URL, "mcpconfig-maven");
