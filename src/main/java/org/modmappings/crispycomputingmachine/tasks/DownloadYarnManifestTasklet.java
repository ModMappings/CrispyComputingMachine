...
@Component
public class DownloadYarnManifestTasklet implements Tasklet, InitializingBean {
    ...
    public RepeatStatus execute(final StepContribution contribution, final ChunkContext chunkContext) throws Exception {
        ...
        File manifestFile = new File(workingDir, Constants.YARN_MAVEN_METADATA_FILE);
        if (manifestFile.exists())
            Assert.state(manifestFile.delete(), "Failed to delete the yarn maven metadata file. It might be in use.");

        Utils.downloadFile(manifestFile.toPath(), Constants.YARN_MAVEN_METADATA_URL, "Yarn-Maven");
