...
@Component
public class MCPConfigMappingReader extends AbstractItemStreamItemReader<ExternalMapping> implements PeekableItemReader<ExternalMapping>
{
    ...
    public MCPConfigMappingReader(final CompositeItemProcessor<String, List<ExternalMapping>> internalMCPConfigMappingReaderProcessor) {
        this.internalVanillaMappingReaderProcessor = internalMCPConfigMappingReaderProcessor;
    }
    ...
    public void open(final ExecutionContext executionContext) {
        ...
        try {
            ...
            File metadataFile = new File(workingDir, Constants.MCPCONFIG_MAVEN_METADATA_FILE);
            Assert.state(metadataFile.exists(), "MCPConfig maven metadata file does not exist!");
            ...
            throw new IllegalStateException("Failed to load maven metadata for MCPConfig mappings.", e);
        }
    }
    ...
    public ExternalMapping read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
        ...
        while (this.versionIterator.hasNext() && (this.currentVersion == null || this.mappingIterator == null || !this.mappingIterator.hasNext())) {
            ...
            LOGGER.info("Loaded: " + this.currentVersion.toString() + " MCPConfig mapping version.");
        ...
        LOGGER.info("Loaded all MCPConfig mapping versions.");
        return null;
    }
